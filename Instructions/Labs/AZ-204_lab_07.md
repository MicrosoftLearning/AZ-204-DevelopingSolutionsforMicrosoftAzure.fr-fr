...
1. Sélectionnez une clé et enregistrez la valeur dans l’une des zones **Chaîne de connexion**. Vous allez utiliser cette valeur plus loin dans ce labo.
...
1. Dans le volet **Créer un coffre de clés**, sous l’onglet **Options de base**, effectuez les actions suivantes, puis sélectionnez l’onglet **Configuration de l’accès** :
...
1. Sous l’onglet **Configuration de l’accès**, modifiez le modèle d’autorisation en **Stratégie d’accès du coffre**.

1. Sélectionnez l’onglet **Vérifier + créer** et passez en revue les options que vous avez sélectionnées lors des étapes précédentes.
...
1. Dans le volet **Créer une stratégie d’accès**, sélectionnez la section **Autorisations** et effectuez les sélections suivantes :
...
1. Dans la zone de texte **Valeur**, construisez une valeur en utilisant la syntaxe suivante : `@Microsoft.KeyVault(SecretUri=<Secret Identifier>)`, où l’espace réservé `<Secret Identifier>` représente l’identificateur de secret que vous avez enregistré précédemment dans cet exercice.
...
1. Exécutez la commande suivante pour **générer** le projet .NET 6 :
...
1. Dans l’Éditeur de code, supprimer tout le contenu.

1. Ajoutez le code suivant dans le fichier **FileParser.cs** :
...
1. Un message d’erreur indiquant que la ressource est introuvable devrait maintenant s’afficher.
   ...
   > **Remarque** : cela est attendu, car notre conteneur est privé.
...
1. Exécutez la commande suivante pour ajouter la version **12.12.0** du package **Azure.Storage.Blobs** à partir de NuGet :
...
1. Passez en revue le fichier **FileParser.cs**, qui doit maintenant avoir :
...
#### Tâche 3 : Écrire du code de Stockage Blob Azure à l’aide du Kit de développement logiciel (SDK) Azure pour .NET

1. Mettez à jour le contenu du fichier **FileParser.cs** avec le code suivant :
...
#### Tâche 4: Déployer et valider l’application Azure Functions
