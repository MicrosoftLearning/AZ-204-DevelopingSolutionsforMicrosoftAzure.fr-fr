...
1. Dans le volet  **Clé d’accès** , examinez chacune des clés (à l’aide du bouton **Afficher**), puis copiez la valeur de l’une des zones  **Chaîne de connexion**  dans le Presse-papiers.
...
1. Dans la fenêtre **Nouveau conteneur**, effectuez les actions suivantes:
   ...
   | Liste **Niveau d’accès anonyme** | Laissez **Privé (aucun accès anonyme)**, puis sélectionnez **Créer**. |
...
1. Dans la section **Configuration**, effectuez les actions suivantes, sélectionnez **Enregistrer**, puis **Continuer**.
   ...
   | Case à cocher **Paramètre d’emplacement de déploiement**          | Conservez la valeur par défaut, puis sélectionnez **OK** pour fermer la boîte de dialogue contextuelle et revenir à la section **Configuration** |
      | Sélectionnez **Enregistrer** dans le menu supérieur           | Cela permet d’enregistrer la valeur de configuration que vous venez d’entrer |
...
1. Entrez la commande suivante, puis sélectionnez Entrée pour déployer le fichier **api.zip** dans l’application web que vous avez créée précédemment dans ce labo:
   ...
   > **Remarque** : remplacez l’espace réservé _\<name-of-your-api-app\>_ par le nom de l’application web que vous avez créée précédemment dans ce labo. Vous avez récemment interrogé le nom de cette application dans les étapes précédentes.
...
1. Entrez la commande suivante, puis sélectionnez Entrée pour déployer le fichier **web.zip** dans l’application web que vous avez créée précédemment dans ce labo:
   ...
   > **Remarque** : remplacez l’espace réservé _\<name-of-your-web-app\>_ par le nom de l’application web que vous avez créée précédemment dans ce labo. Vous avez récemment interrogé le nom de cette application dans les étapes précédentes.
