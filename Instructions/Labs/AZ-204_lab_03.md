...
Enfin, pour garantir la conformité aux réglementations d’entreprise, vous allez implémenter un accès sécurisé en générant des signatures d’accès partagé et des stratégies de cycle de vie des données. Vous testerez également l’accès au contenu via un site web statique.
...
1. Dans le terminal, exécutez la commande suivante pour importer la version 12.12.0 d’**Azure.Storage.Blobs** à partir de NuGet :
