...
1. Dans la fenêtre de navigateur qui s’ouvre, accédez à l’URL `http://localhost` et ajoutez le chemin relatif **/weatherforecast** de votre API web.
    ...
    > **Remarque** : L’URL complète est `http://localhost:[port-number]/weatherforecast`, où l’espace réservé `[port-number]` identifie le numéro de port auquel l’application web est accessible via le protocole HTTPS.
...
1. Dans la fenêtre **Visual Studio Code**, dans le volet **Explorateur**, accédez au répertoire **bin\Debug\net8.0**.
    ...
    > **Remarque** : pour vérifier que vous êtes dans le répertoire **bin\Debug\net8.0**, dans Visual Studio Code, dans le menu Fichier, sélectionnez Ouvrir un dossier et accédez à **Allfiles (F):\\Allfiles\\Labs\\11\\Starter\\Api\\bin\\Release\\net8.0\win-x86\publish**, puis choisissez **Sélectionner un dossier**.
