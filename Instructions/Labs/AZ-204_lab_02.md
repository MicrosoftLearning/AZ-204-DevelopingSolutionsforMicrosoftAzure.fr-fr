...
ms.locfileid: "145908265"
---
# <a name="lab-02-implement-task-processing-logic-by-using-azure-functions"></a>Lab 02 : Implémenter une logique de traitement des tâches à l’aide d’Azure Functions
...
#### <a name="task-2-create-an-azure-storage-account"></a>Tâche 2 : Créer un compte de stockage Azure
...
1. Sous l’onglet **Hébergement**, effectuez les actions suivantes, puis sélectionnez **Vérifier + créer** :
    ...
    La capture d’écran suivante affiche les paramètres configurés dous l’onglet **Hébergement** dans le volet **Créer une application de fonction**.

    ![Capture d’écran montrant les paramètres configurés sous l’onglet Hébergement dans le volet Créer une application de fonction ](./media/l02_create_a_function_app_hosting_tab.png)
...
1. Dans la méthode **Run**, entrez la ligne de code suivante pour journaliser un message fixe :
...
#### <a name="task-3-write-http-triggered-and-blob-inputted-function-code"></a>Tâche 3 : Écrire un code de fonction déclenché par HTTP et entré par blob
...
#### <a name="task-4-register-azure-storage-blob-extensions"></a>Tâche 4 : Inscrire les extensions Azure Storage Blob
...
1. Dans la boîte de dialogue contextuelle qui s’affiche, effectuez les actions suivantes : <!--Conan, as there is only sub step here, can we just combine the two as follows: In the popup dialog that appears, in the **HTTP method** list, select **GET**.-->
